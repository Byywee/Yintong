...
elseif ($action == 'order_detail')
{
	...
	foreach($goods_list AS $key=>$val)
	{
		...
		$sql	=	"select add_time from ".$ecs->table('order_info')." where order_id='".$_GET['order_id']."'";
		$goods_list[$key]['add_time']	=	$db->getOne($sql);
		//back_sale_num -->退货数量
		//$goods_list[$key]['back_sales_num']	=	$goodslist['back_sales_num'];		
	}
	...
	foreach($goods_list AS $key=>$val)
	{
		$sql  = " select sale_return_able_date,sale_return_able"; 
        $sql .= " from ".$ecs->table('order_goods')." eo,".$ecs->table('goods')." eg ";
		$sql .= " where eo.goods_id = eg.goods_id and eo.goods_id = ".$goods_list[$key]['goods_id'];
		...
		$info['add_time'] = date('Y-m-d',$goods_list[$key]['add_time']);
		$info['now'] = date('Y-m-d',time());
		
				
		$sql_order_deal_status  = " select deal_sale_return_status from ".$ecs->table('order_goods');
		$sql_order_deal_status .= " where order_id = ".$_GET['order_id'];
		$sql_order_deal_status .= " and goods_id = ".$goods_list[$key]['goods_id']."";
...
elseif($action == 'sales_return')
{
	...
	$sql_get_order_goods_number = "select goods_number from ".$ecs->table('order_goods')." where order_id = ".$_REQUEST['order_id']." and goods_id = ".$_POST['goods_id']."";
	$goods_number = $db->getOne($sql_get_order_goods_number);
	$order_info_return = $db->getRow($sql_get_order_info);
	//print('<br />');
	//print_r($order);
	//print('<br />');
	$user_info_return = $db->getRow($sql_get_user_info);
	$user_info_return_addr = $db->getRow($sql_get_user_info_addr);
	//print_r($user);
	//die;
	$backMoney = $_POST['goods_cur_price'] * $_POST['sales_return_num']; 
	//echo "<br />".$backMoney;
	if($_POST['sales_return_num'] > $goods_number)
	{
		if ((!isset($back_act)|| empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
    	{
       		$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? './index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    	}
   		show_message('请注意，退货数量不能大于商品数量！！', array($_LANG['back_up_page'],$_LANG['back_home_lnk'],), array('user.php?act=order_detail&order_id='.$_POST['order_id'],$back_act), 'warnning');
	}
	if($_POST['sales_return_num'] <= 0)
	{
		if ((!isset($back_act)|| empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
    	{
       		$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? './index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    	}
   		show_message('请注意，退货数量不合法！！', array($_LANG['back_up_page'],$_LANG['back_home_lnk'],), array('user.php?act=order_detail&order_id='.$_POST['order_id'],$back_act), 'warnning');
	}
	...
	//	echo "<br / >sql_query_info==".$sql_query_info;
		...
		if($goods_amount > $backMoney)
		{
			...
			$sql_order .= " surplus=surplus-".$backMoney.",";
			$sql_order .= " money_paid=money_paid-".$backMoney;
	...
	//	print($sql_up."<br />");
		...
		//更新退货订单表
		$now = time();
		$sql_insert_back_order  = " insert into ".$ecs->table('back_order');
		$sql_insert_back_order .= " (back_id,order_sn,order_id,add_time,shipping_id,shipping_name,user_id,action_user,consignee,";
		$sql_insert_back_order .= "	address,country,province,city,district,";
		$sql_insert_back_order .= " sign_building,email,zipcode,tel,mobile,best_time,postscript,";
		$sql_insert_back_order .= " how_oos,insure_fee,shipping_fee,return_time,agency_id,deal_sale_return_status)";
		$sql_insert_back_order .= " values(NULL,'".$order_info_return['order_sn']."','";
		$sql_insert_back_order .= $order_info_return['order_id']."','".$order_info_return['add_time']."','";	 
		$sql_insert_back_order .= $order_info_return['shipping_id']."','".$order_info_return['shipping_name'];
		$sql_insert_back_order .= "','".$_REQUEST['user_id']."','','".$user_info_return_addr['consignee']."','";
		$sql_insert_back_order .= $user_info_return_addr['address']."','".$user_info_return_addr['".country."'];
		$sql_insert_back_order .= "','".$user_info_return_addr['province']."','".$user_info_return_addr['city'];
		$sql_insert_back_order .= "','".$user_info_return_addr['district']."','";
		$sql_insert_back_order .= $user_info_return_addr['sign_building']."','".$user_info_return_addr['email'];
		$sql_insert_back_order .= "','".$user_info_return_addr['zipcode']."','".$user_info_return['home_phone'];
		$sql_insert_back_order .= "','".$user_info_return['mobile_phone']."','".$order_info_return['best_time'];
		$sql_insert_back_order .= "','".$order_info_return['postscript']."','".$order_info_return['how_oos'];
		$sql_insert_back_order .= "','".$order_info_return['insure_fee']."','".$order_info_return['shipping_fee']."','".$now."','".$order_info_return['agency_id']."',0)";
	//	print($sql_insert_back_order."<br />");die;
		$db->query($sql_insert_back_order);
		
		$sql_get_back_id  = " select back_id from ".$ecs->table('back_order');
		$sql_get_back_id .= " where return_time = '".$now."'";
		
		$new_back_id = $db->getOne($sql_get_back_id);
		
		//更新退货商品表
		$sql_get_product_good_sn  = " select goods_sn gn,product_sn pn ";
		$sql_get_product_good_sn .= " from ecs_goods eg,ecs_products ep ";
		$sql_get_product_good_sn .= " where eg.goods_id = ep.goods_id ";
		$sql_get_product_good_sn .= " and eg.goods_id = ".$_POST['goods_id'];
		$sn = $db->getRow($sql_get_product_good_sn);
		
		$sql_insert_back_goods  = " insert into ".$ecs->table('back_goods');
		$sql_insert_back_goods .= " (back_id, goods_id, goods_name,send_number,product_sn,goods_sn)";
		$sql_insert_back_goods .= " values(".$new_back_id.",".$_POST['goods_id'];
	    $sql_insert_back_goods .= " ,'".$_POST['goods_name']."',".$_POST['sales_return_num'].",'".$sn['pn']."','".$sn['gn']."')";	
		//
		//print($sql_insert_back_goods."<br />");die;
		$db->query($sql_insert_back_goods);
		...
		if ((!isset($back_act)|| empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
    	{
   		...
   		show_message($_LANG['sr_operate'], array($_LANG['back_up_page'],$_LANG['back_home_lnk'],), array('user.php?act=order_detail&order_id='.$_POST['order_id'],$back_act), 'info');
	}
}
?>