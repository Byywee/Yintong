...
elseif($_REQUEST['act'] == 'back_cancel')
{
	...
	{	
			...
			$sql_update_back_goods .= " send_number = send_number - ".$_REQUEST['back_num'];
			$sql_update_back_goods .= " where back_id = ".$_REQUEST['back_id'];
			//$db->query($sql_update_back_goods);
			
			$sql_update_back_order  = " update ecs_back_order set deal_sale_return_status=1 where back_id=".$_REQUEST['back_id'];
			$db->query($sql_update_back_order);
			
			$sql_update_order_goods  = " update ecs_order_goods ";
			$sql_update_order_goods .= " set back_sales_num = back_sales_num - ".$_REQUEST['back_num']." ,";
			$sql_update_order_goods .= " deal_sale_return_status = 1 ";
			$sql_update_order_goods .= " where order_id = ".$_REQUEST['order_id'];
			$db->query($sql_update_order_goods);
			
		//	print($sql_del_back_order."<br />");
		//	print($sql_update_back_order."<br />");
		//	print($sql_update_order_goods."<br />");die;
		
		$links[] = array('href' => 'order.php?act=back_list', 'text' => $_LANG['02_back_list']);
		sys_msg($_LANG['act_ok'], 0, $links);
	}
	//ajax获取客户沟通备注
	elseif($_REQUEST['note'] == 'get')
	{
		$sql_get_custome_note  = " select custom_com_note from ecs_order_goods";
		$sql_get_custome_note .= " where order_id in (select order_id from ecs_order_info ";
		$sql_get_custome_note .= " where order_sn = '".$_GET['order_sn']."')";
		$cus_note = $db->getOne($sql_get_custome_note);
		echo $cus_note;
	}
	//ajax进行客户沟通备注更新
	elseif($_REQUEST['note'] == 'note')
	{
		$sql_add_custome_note  = " update ecs_order_goods set ";
		$sql_add_custome_note .= " custom_com_note = '".$_GET['custom_com_note']."'";
		$sql_add_custome_note .= " where order_id in (select order_id from ecs_order_info ";
		$sql_add_custome_note .= " where order_sn = '".$_GET['order_sn']."')";
		$db->query($sql_add_custome_note);
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, o.back_sales_num, o.deal_sale_return_status
...
elseif ($_REQUEST['act'] == 'back_info')
{
	...
	$deal_sale_return_status = intval(trim($_REQUEST['deal_sale_return_status']));
	...
	$sql_custome_note .= " and order_id in (select order_id from ecs_back_order where back_id = ".$back_id.")";
	
	$back_order['custom_com_note'] = $GLOBALS['db']->getOne($sql_custome_note);
    /* 模板赋值 */
	$smarty->assign('deal_sale_return_status',$deal_sale_return_status);
...
//	print_r($_POST);
//	die;
	$sql_get_num  = " select sum(goods_number) as order_goods_num, sum(back_sales_num) as order_back_goods_num ";
	$sql_get_num .= " from ecs_order_goods where order_id in (select order_id from ecs_back_order where ";
	$sql_get_num .= " back_id=".$_REQUEST['back_id'].")";
	//die($sql_get_num);
	$total_num = $db->getRow($sql_get_num);
	//print_r($total_num);die;
	if($total_num['order_goods_num'] > $total_num['order_back_goods_num'])
	{
		//更新退货订单从列表ecs_back_order
		
		
		//更新订单商品表ecs_order_goods
	//	$sql_get_ecs_order_goods_num  = " select send_number from ".$ecs->table('order_goods');
	//	$sql_get_ecs_order_goods_num .= " where goods_id = ".$_POST['goods_id'];
		
	//	$goods_num = $db->getOne($sql_get_ecs_order_goods_num);
		$sql_update_ecs_order_goods  = " update ".$ecs->table('order_goods');
		$sql_update_ecs_order_goods .= " set back_sales_num = back_sales_num-".$_POST['send_number'].",";
		$sql_update_ecs_order_goods .= " goods_number = goods_number - ".$_POST['send_number'].",";
		$sql_update_ecs_order_goods .= " deal_sale_return_status = 1 ";
		$sql_update_ecs_order_goods .= " where goods_id = ".$_POST['goods_id']." ";
		$sql_update_ecs_order_goods .= " and order_id = ".$_POST['order_id'];		
		
		//更新退货商品列表ecs_back_goods
		$sql_update_ecs_back_goods  = " update ".$ecs->table('back_order');
	//	$sql_update_ecs_back_goods .= " set send_number = send_number - ".$_POST['send_number']."";
		$sql_update_ecs_back_goods .= " set deal_sale_return_status = 1 ";
		$sql_update_ecs_back_goods .= " where back_id = ".$_POST['back_id'];
		
		//获取所退商品的价格
		$sql_get_price = " select goods_price from ecs_order_goods where goods_id = ".$_POST['goods_id'];
		$goods_price = $db->getOne($sql_get_price);
		
		$back_money = $goods_price * $_POST['send_number'];
		//更新订单价格
		$sql_update_order_info  = " update ecs_order_info set ";
		$sql_update_order_info .= " goods_amount=goods_amount-".$back_money.",";
		$sql_update_order_info .= " surplus=surplus-".$back_money.",";
		$sql_update_order_info .= " order_amount=order_amount-".$back_money.",";
		$sql_update_order_info .= " money_paid=money_paid-".$back_money;
		$sql_update_order_info .= " where order_id = ".$_POST['order_id']; 
		
		//退还用户的相应商品的总金额
		$sql_update_ecs_users_money  = " update ".$ecs->table('users');
		$sql_update_ecs_users_money .= " set user_money = user_money + ".$back_money." ";
		$sql_update_ecs_users_money .= " where user_id in ( select user_id from ecs_back_order where ";
		$sql_update_ecs_users_money .= " back_id = ".$_POST['back_id'].")";
		
		$sql_update_goods_inventory  = " update ecs_goods set goods_number = goods_number + ".$_POST['send_number'];
		$sql_update_goods_inventory .= " where goods_id = ".$_POST['goods_id'];
		
		$sql_get_good_price = "select goods_price from ecs_order_goods where order_id = ".$_POST['order_id']." and goods_id = ".$_POST['goods_id'];
		$good_price  = $db->getOne($sql_get_good_price);
		$back_money = $good_price * $_POST['send_number'];
		$sql_back_user_money  = " update ecs_users set user_money = user_money + ".$back_money;
		$sql_back_user_money .= " where user_id in ( select user_id from ecs_back_order where back_id = ".$_POST['back_id'].")";
		if($_POST['change_inventory'] == 'change_inventory')
		{
		//	echo "<br />".$sql_update_goods_inventory;
			$db->query($sql_update_goods_inventory);
		}
	//	echo $sql_back_user_money;
	//	die;
	
		$db->query($sql_update_ecs_order_goods);
		$db->query($sql_update_ecs_back_goods);
		$db->query($sql_update_order_info);
		$db->query($sql_back_user_money);
		$order_goods_num_total = $total_num['order_goods_num'];
	}
	elseif($total_num['order_goods_num'] == $total_num['order_back_goods_num'])
	{
		$sql_update_ecs_order_goods  = " update ".$ecs->table('back_order');
		$sql_update_ecs_order_goods .= " set deal_sale_return_status = 1 ";
		$sql_update_ecs_order_goods .= " where back_id = ".$_POST['back_id'];
		
		$sql_update_order_status  = " update ecs_order_info set order_status = 3"; 
		$sql_update_order_status .= " where order_id in (select order_id from ecs_back_order where ";
		$sql_update_order_status .= " back_id=".$_REQUEST['back_id'].")";
	...
	//	die;
		$db->query($sql_update_ecs_order_goods);
		$db->query($sql_update_order_status);
		$db->query($sql_back_user_money);
	}
...
function back_list()
{
	...
	//	$GLOBALS['db']->query($sql_del_send_number);
        ...
        /* 记录总数 */
       // $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('back_goods') . $where;
		...
		// $sql  = " SELECT rec_id,back_id,goods_id,product_id,product_sn,goods_name,brand_name, ";
		// $sql .= " goods_sn,is_real,send_number,goods_attr FROM " . $GLOBALS['ecs']->table("back_goods");
		// $sql .= $where." ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'];
		 $sql_info .= " LIMIT ".($filter['page']-1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";

        set_filter($filter, $sql_info);
    }
    else
    {
        $sql    = $result['sql'];
...
function back_order_info($back_id)
{
	...
	$sql .= " WHERE back_id = '".$back_id."' ".$where." LIMIT 0, 1";
